...
class ConcurrentBlockAnalysis(
    ...
    /** 块级局部名（顶层 YxVarDecl）。 */
    internal fun blockLocals(block: YxBlock): Set<String> = block.statements.mapNotNull { (it as? YxVarDecl)?.name }.toSet()
    ...
    private fun readsBlockLocal(
        ...
        when (stmt) {
            ...
            is YxExprStmt -> {
                readsBlockLocalExpr(stmt.expr, locals, bound)
            }
            ...
            is YxIf -> {
                readsBlockLocalExpr(stmt.condition, locals, bound) ||
            ...
            is YxFor -> {
                readsBlockLocalExpr(stmt.iterable, locals, bound) ||
                    readsBlockLocal(stmt.body, locals, bound)
            }

            is YxWhen -> {
                stmt.branches.any { readsBlockLocal(it.body, locals, bound) }
            }
    ...
    private fun readsBlockLocalExpr(
        expr: YxExpr,
        locals: Set<String>,
        bound: MutableSet<String>,
    ): Boolean =
        when (expr) {
            is YxIdentifier -> {
                hitBlockLocal(expr, locals, bound)
            }

            is YxMemberAccess -> {
                readsBlockLocalExpr(expr.receiver, locals, bound)
            }

            is YxCall -> {
                readsBlockLocalExpr(expr.callee, locals, bound) || expr.args.any { readsBlockLocalExpr(it, locals, bound) }
            }

            is YxTypeCall -> {
                expr.args.any { readsBlockLocalExpr(it, locals, bound) }
            }

            is YxBinary -> {
                readsBlockLocalExpr(expr.left, locals, bound) || readsBlockLocalExpr(expr.right, locals, bound)
            }

            is YxUnary -> {
                readsBlockLocalExpr(expr.operand, locals, bound)
            }

            is YxParen -> {
                readsBlockLocalExpr(expr.expr, locals, bound)
            }
            ...
            is YxRange -> {
                readsBlockLocalExpr(expr.from, locals, bound) || readsBlockLocalExpr(expr.to, locals, bound)
            }

            is YxStringTemplate -> {
                expr.parts.any { readsBlockLocalExpr(it, locals, bound) }
            }

            is YxIndexExpr -> {
                readsBlockLocalExpr(expr.base, locals, bound) || readsBlockLocalExpr(expr.index, locals, bound)
            }

            is YxAssign -> {
                readsBlockLocalExpr(expr.target, locals, bound) || readsBlockLocalExpr(expr.value, locals, bound)
            }
            ...
            is YxIfExpr -> {
                readsBlockLocalExpr(expr.condition, locals, bound) ||
                    readsBlockLocalExpr(expr.thenExpr, locals, bound) ||
                    readsBlockLocalExpr(expr.elseExpr, locals, bound)
            }

            is YxWhenExpr -> {
                (expr.subject?.let { readsBlockLocalExpr(it, locals, bound) } ?: false) ||
    ...
    private fun checkStmt(
        ...
        when (stmt) {
            ...
            is YxExprStmt -> {
                checkExpr(stmt.expr, locals, bound)
            }

            is YxReturn -> {
                throw IllegalStateException("async/parallel 块内不支持 return（块编译为 lambda，v0.1）")
            }

            is YxBreak, is YxContinue -> {
                if (loopDepth == 0) {
            ...
            is YxBlock -> {
                stmt.statements.forEach { checkStmt(it, locals, bound, loopDepth) }
            }
            ...
            is YxWhen -> {
                stmt.branches.forEach { checkStmt(it.body, locals, bound, loopDepth) }
            }
    ...
    private fun checkExpr(
        ...
        when (expr) {
            ...
            // 读外部局部 = 按值捕获（合法）
            is YxMemberAccess -> {
                checkExpr(expr.receiver, locals, bound)
            }
            ...
            is YxUnary -> {
                checkExpr(expr.operand, locals, bound)
            }

            is YxParen -> {
                checkExpr(expr.expr, locals, bound)
            }
            ...
            is YxStringTemplate -> {
                expr.parts.forEach { checkExpr(it, locals, bound) }
            }
