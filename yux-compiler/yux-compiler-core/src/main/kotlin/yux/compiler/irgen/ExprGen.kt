...
class ExprGen(
    ...
    fun gen(
        ...
        when (expr) {
            is YxIntLiteral -> {
                IrExpr.Const(decodeInt(expr.text))
            }

            is YxFloatLiteral -> {
                IrExpr.Const(decodeFloat(expr.text))
            }

            is YxCharLiteral -> {
                IrExpr.Const(decodeChar(expr.text))
            }

            is YxStringLiteral -> {
                IrExpr.Const(decodeString(expr.text))
            }

            is YxRawStringLiteral -> {
                IrExpr.Const(expr.text.removePrefix("\"\"\"").removeSuffix("\"\"\""))
            }

            is YxStringText -> {
                IrExpr.Const(expr.text)
            }

            is YxBoolLiteral -> {
                IrExpr.Const(expr.value)
            }
            ...
            // M4 简化：super 视为 this（S-8.7 继承语义后置）
            is YxParen -> {
                gen(expr.expr, g, fileScope)
            }
            ...
            is YxTypeReference -> {
                IrExpr.ClassLiteral(irGen.resolveIrType(expr.type, fileScope))
            }

            is YxMemberAccess -> {
                genMemberAccess(expr, g, fileScope)
            }

            is YxCall -> {
                genCall(expr, g, fileScope)
            }
            ...
            is YxBinary -> {
                genBinary(expr, g, fileScope)
            }

            is YxUnary -> {
                genUnary(expr, g, fileScope)
            }

            is YxIfExpr -> {
                genIfExpr(expr, g, fileScope)
            }

            is YxWhenExpr -> {
                genWhenExpr(expr, g, fileScope)
            }

            is YxIndexExpr -> {
                genIndexExpr(expr, g, fileScope)
            }

            is YxLambda -> {
                genLambda(expr, g, fileScope)
            }

            is YxBlockLambda -> {
                genBlockLambda(expr, g, fileScope)
            }

            is YxStringTemplate -> {
                IrExpr.StringTemplate(
                    expr.parts.map { if (it is YxStringText) IrExpr.Const(it.text) else gen(it, g, fileScope) },
                )
            }

            is YxIs -> {
                IrExpr.IsType(gen(expr.expr, g, fileScope), irGen.resolveIrType(expr.type, fileScope))
            }

            is YxAs -> {
                IrExpr.Convert(gen(expr.expr, g, fileScope), irGen.resolveIrType(expr.type, fileScope))
            }

            is YxThrow -> {
                error("IRGen: throw 仅支持语句位置（M4）")
            }

            is YxAwait -> {
                genAwait(expr, g, fileScope)
            }
            ...
            is yux.compiler.ast.YxAssign -> {
                genAssignValue(expr, g, fileScope)
            }
        }
    ...
    private fun genIdentifier(
        ...
        return when (sym) {
            ...
            is FunctionSymbol -> {
                val method =
                    irGen.functionMethods[sym]
                        ?: error("IRGen: 函数未登记: ${sym.name}")
                IrExpr.Lambda(IrMethodRef(method))
            }

            else -> {
                error("IRGen: 未解析的标识符: ${expr.name}")
            }
        }
    }

    private fun genMemberAccess(
        expr: YxMemberAccess,
        g: MethodGen,
        fileScope: FileScope,
    ): IrExpr {
    ...
    private fun resolveMemberExpr(
        ...
        when {
            rt is SemaType.Declared && rt.symbol is YxClassSymbol -> {
                ...
                resolveMemberExprJvm(receiver, rt, name, args, argTypes, isSuper)
            }
        }
    ...
    private fun resolveMemberExprJvm(
        ...
        return resolver
            .resolveInstanceMethod(rt, name, argTypes)
    ...
    private fun genCall(
        ...
        if (asyncSym != null && asyncSym.isAsync && irGen.isInAsyncContext()) {
            return genSuspendCall(asyncSym, call.callee, padDefaultArgs(asyncSym, args, g, fileScope), g, fileScope)
        }
        return when (val callee = call.callee) {
            ...
            is YxMemberAccess -> {
                genMemberCall(callee, args, argTypes, g, fileScope)
            }
    ...
    /**
     * S-5.5.5 参数默认值合成（T-M12 缺陷修复）：调用实参少于形参时，在调用点生成缺失
     * 参数的默认值表达式。默认值可引用更早形参（如 `fun f(a:Int, b:Int = a)`）——
     * 将已提供实参物化为以形参名命名的临时局部，使默认表达式中的标识符按名解析到该局部；
     * 默认值就地生成并写入同名局部。返回补齐后的实参列表。
     */
    fun padDefaultArgs(
        sym: FunctionSymbol,
        args: List<IrExpr>,
        ...
        val decl = sym.decl ?: return args // 内置函数（print 等）decl==null 且无默认值
        val result = args.toMutableList()
        val recvOffset = if (sym.receiverType != null) 1 else 0
        g.enterScope()
        try {
            for (i in result.indices) {
                val p = params[i]
                val local = g.newLocal(p.name, TypeBridge.toIr(p.type))
                g.declareLocal(p.name, local)
                g.emit(IrStmt.LocalAssign(local, result[i]))
                result[i] = IrExpr.LocalRead(local)
            }
            for (i in result.size until params.size) {
                val p = params[i]
                if (!p.hasDefault) {
                    error("IRGen: 实参数目不匹配: 参数 '${p.name}' 缺少实参且无默认值（sema 应已拒绝）")
                }
                val defaultExpr =
                    decl.params.getOrNull(i - recvOffset)?.defaultValue
                        ?: error("IRGen: 参数 '${p.name}' 默认值缺失")
                val local = g.newLocal(p.name, TypeBridge.toIr(p.type))
                g.declareLocal(p.name, local)
                g.emit(IrStmt.LocalAssign(local, gen(defaultExpr, g, fileScope)))
                result.add(IrExpr.LocalRead(local))
            }
        } finally {
            g.exitScope()
        }
    ...
    private fun genMemberCall(
        ...
        if (callee.receiver is YxTypeReference) {
            val static =
                resolver.resolveStaticMethod(receiverType, callee.name, argTypes)
            ...
            if (extSym is FunctionSymbol && extSym.receiverType != null) {
                val method =
                    irGen.functionMethods[extSym]
                        ?: error("IRGen: 扩展函数未登记: ${extSym.name}")
                val receiver = castSmartReceiver(gen(callee.receiver, g, fileScope), receiverType)
                // 扩展函数 receiver 是形参 0（S-5.5.5 默认值可引用它），补齐后一并传入
                val padded = padDefaultArgs(extSym, listOf(receiver) + args, g, fileScope)
                result = IrExpr.Invoke(IrMethodRef(method), null, padded)
            } else {
                val receiver = castSmartReceiver(gen(callee.receiver, g, fileScope), receiverType)
                val fnSym =
                    ((receiverType as? SemaType.Declared)?.symbol as? YxClassSymbol)
                        ?.functionsIncludingSuper(callee.name)
                        ?.firstOrNull()
                val padded = if (fnSym != null) padDefaultArgs(fnSym, args, g, fileScope) else args
                result = resolveMemberExpr(receiver, receiverType, callee.name, padded, argTypes, isSuper = callee.receiver is YxSuper)
     ...
     */
    private fun builtinCall(sym: FunctionSymbol): IrCallable =
        IrJvmCall(
            name = sym.name,
    ...
    private fun defaultConst(type: IrType): IrExpr =
        when (val t = type.nonNull()) {
            is IrType.Basic -> {
                when (t.name) {
                    "Int" -> IrExpr.Const(0)
    ...
    private fun genLambda(
        expr: YxLambda,
        g: MethodGen,
        fileScope: FileScope,
    ): IrExpr {
        val fnType =
            irGen.exprTypes[expr] as? SemaType.Function
    ...
    private fun captureLocal(
        g: MethodGen,
        name: String,
    ): IrLocal = g.lookupLocal(name) ?: error("IRGen: 捕获变量未登记: $name")
    ...
    private fun symbolType(sym: Symbol): SemaType? =
        when (sym) {
    ...
    internal fun collectCaptures(
        ...
        when (node) {
            ...
            is YxMemberAccess -> {
                collectCaptures(node.receiver, bound, captures)
            }
            ...
            is YxUnary -> {
                collectCaptures(node.operand, bound, captures)
            }

            is YxParen -> {
                collectCaptures(node.expr, bound, captures)
            }
            ...
            is YxStringTemplate -> {
                node.parts.forEach { collectCaptures(it, bound, captures) }
            }
            ...
            is YxReturn -> {
                node.value?.let { collectCaptures(it, bound, captures) }
            }
            ...
            is YxThis, is YxSuper, is YxTypeReference, is YxBreak, is YxContinue,
            -> {
                Unit
            }
    ...
    private fun genAssignValue(
        expr: yux.compiler.ast.YxAssign,
        g: MethodGen,
        fileScope: FileScope,
    ): IrExpr =
        when (val t = expr.target) {
            ...
            is YxMemberAccess -> {
                ...
                val prop =
                    (receiverType.symbol as YxClassSymbol).propertyIncludingSuper(t.name)
                ...
                val receiverLocal =
                    if (t.receiver is YxTypeReference) {
                        null
                    } else {
            ...
            is YxIndexExpr -> {
                genIndexAssign(t, expr.value, g, fileScope)
            }
