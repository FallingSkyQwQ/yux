...
object AstPrinter {
    ...
    fun dump(decls: List<YxDecl>): String = buildString {
        appendLine("program")
    ...
    private fun appendLine(sb: StringBuilder, depth: Int, line: String) {
        sb.append(indent(depth)).append(line).append('\n')
    }

    private fun renderDecl(decl: YxDecl, depth: Int): String = buildString {
        when (decl) {
            is YxPackage -> appendLine(this, depth, "package ${decl.name}")
            is YxImport -> appendLine(this, depth, if (decl.star) "import ${decl.qualifiedName.joinToString(".")}.*" else "import ${decl.qualifiedName.joinToString(".")}")
            is YxClass -> {
                decl.annotations.forEach { appendLine(this, depth, renderAnnotation(it)) }
                val vis = visibilityPrefix(decl.visibility)
                appendLine(this, depth, "$vis${decl.name}${renderTypeParams(decl.typeParams)}${renderSuper(decl.superType, decl.interfaces)}")
    ...
    private fun renderClassMember(member: YxClassMember, depth: Int): String = buildString {
        when (member) {
            is YxFunction -> append(renderFunction(member, depth))
            is YxProperty -> append(renderProperty(member, depth))
            is YxInitBlock -> {
    ...
    private fun renderFunction(funDecl: YxFunction, depth: Int): String = buildString {
    ...
    private fun renderProperty(prop: YxProperty, depth: Int): String = buildString {
    ...
    private fun renderAccessor(accessor: YxAccessor, depth: Int): String = buildString {
    ...
    private fun renderAnnotation(annotation: YxAnnotation): String {
        val args = if (annotation.args.isNotEmpty()) {
    ...
    private fun renderSuper(superType: YxType?, interfaces: List<YxType>): String {
    ...
    private fun visibilityPrefix(vis: YxVisibility): String = when (vis) {
        YxVisibility.PUBLIC -> ""
    ...
    private fun renderStmt(stmt: YxStmt, depth: Int): String = buildString {
        when (stmt) {
            is YxBlock -> append(renderBlock(stmt, depth))
            is YxVarDecl -> {
            ...
            is YxAssign -> appendLine(this, depth, "assign ${renderExpr(stmt.target)} ${stmt.op} ${renderExpr(stmt.value)}")
            is YxExprStmt -> appendLine(this, depth, "expr ${renderExpr(stmt.expr)}")
            is YxIf -> {
            ...
            is YxBreak -> appendLine(this, depth, "break")
            is YxContinue -> appendLine(this, depth, "continue")
            is YxTry -> {
    ...
    private fun renderWhenBranch(branch: YxWhenBranch, depth: Int): String = buildString {
    ...
    private fun renderCatch(catch: YxCatchClause, depth: Int): String = buildString {
    ...
    private fun renderBlock(block: YxBlock, depth: Int): String = buildString {
    ...
    private fun renderType(type: YxType): String = when (type) {
        is YxNamedType -> {
        ...
        is YxFunctionType -> "(${type.params.joinToString(", ") { renderType(it) }}) -> ${renderType(type.ret)}"
    }

    // ── 表达式（单行）─────────────────────────────────────────────────────────

    private fun renderExpr(expr: YxExpr): String = when (expr) {
        is YxIntLiteral -> "Int(${expr.text})"
        is YxFloatLiteral -> "Float(${expr.text})"
        is YxCharLiteral -> "Char(${expr.text})"
        is YxBoolLiteral -> if (expr.value) "Bool(true)" else "Bool(false)"
        is YxNullLiteral -> "Null"
        is YxRawStringLiteral -> "Raw(${quote(expr.text)})"
        is YxStringLiteral -> "Str(${quote(expr.text)})"
        is YxStringText -> "Text(${quote(expr.text)})"
        is YxStringTemplate -> "Template[${expr.parts.joinToString(", ") { renderExpr(it) }}]"
        is YxIdentifier -> "Id(${expr.name})"
        is YxThis -> "This"
        is YxSuper -> "Super"
        is YxTypeReference -> "TypeRef(${renderType(expr.type)})"
        is YxMemberAccess -> "Get(${renderExpr(expr.receiver)}, ${expr.name})"
        is YxCall -> {
            val marker = if (expr.noParen) "CallNP" else "Call"
            "$marker(${renderExpr(expr.callee)}, [${expr.args.joinToString(", ") { renderExpr(it) }}])"
        }
        is YxTypeCall -> "New(${renderType(expr.type)}, [${expr.args.joinToString(", ") { renderExpr(it) }}])"
        is YxBinary -> "Binary(${expr.op}, ${renderExpr(expr.left)}, ${renderExpr(expr.right)})"
        is YxUnary -> "Unary(${expr.op}, ${renderExpr(expr.operand)})"
        is YxLambda -> "Lambda([${expr.params.joinToString(", ")}], ${renderExpr(expr.body)})"
    ...
    private fun inlineBlock(block: YxBlock): String =
        block.statements.joinToString("; ") { inlineStmt(it) }

    private fun inlineStmt(stmt: YxStmt): String = when (stmt) {
        is YxBlock -> "{ ${inlineBlock(stmt)} }"
        is YxExprStmt -> renderExpr(stmt.expr)
        is YxVarDecl -> {
            val init = stmt.initializer?.let { " = ${renderExpr(it)}" } ?: ""
            "var ${stmt.name}$init"
        }
        is YxAssign -> "${renderExpr(stmt.target)} ${stmt.op} ${renderExpr(stmt.value)}"
        is YxReturn -> "return${stmt.value?.let { " ${renderExpr(it)}" } ?: ""}"
        is YxIf -> "if ${renderExpr(stmt.condition)} ${inlineStmt(stmt.thenBranch)}" +
            (stmt.elseBranch?.let { " else ${inlineStmt(it)}" } ?: "")
        else -> renderStmt(stmt, 0).trim()
    }
