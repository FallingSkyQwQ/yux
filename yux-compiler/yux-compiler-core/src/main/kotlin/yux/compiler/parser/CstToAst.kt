...
class CstToAst {
    /** 转换整个程序。 */
    fun convert(program: CstProgram): List<YxDecl> = program.decls.mapNotNull { convertTopLevel(it) }

    private fun convertTopLevel(decl: CstDecl): YxDecl? =
        when (decl) {
            is CstPackageDecl -> {
                YxPackage(decl.name.joinToString(".") { it.text }, decl.span)
            }

            is CstImportDecl -> {
                YxImport(decl.name.map { it.text }, decl.star != null, decl.span)
            }
            ...
            is CstFunctionDecl -> {
                convertFunction(decl)
            }

            is CstPropertyDecl -> {
                convertProperty(decl, isTopLevel = true)
            }
    ...
    private fun convertClassMember(member: CstClassMember): YxClassMember? =
        when (member) {
    ...
    private fun convertParameter(param: CstParameter): YxParameter =
        YxParameter(
    ...
    private fun convertAccessor(accessor: CstAccessor): YxAccessor =
        YxAccessor(
    ...
    private fun convertAnnotation(annotation: CstAnnotation): YxAnnotation =
        YxAnnotation(
    ...
    private fun convertVisibility(modifier: yux.compiler.lexer.Token?): YxVisibility =
        when (modifier?.text) {
    ...
    private fun convertType(type: CstType): YxType =
        when (type) {
            is CstNamedType -> {
                YxNamedType(
    ...
    private fun convertStmt(stmt: CstStmt): YxStmt =
        when (stmt) {
            is CstBlock -> {
                convertBlock(stmt)
            }
            ...
            is CstAssignStmt -> {
                YxAssign(convertExpr(stmt.target), stmt.op.text, convertExpr(stmt.value), stmt.span)
            }

            is CstExprStmt -> {
                YxExprStmt(convertExpr(stmt.expr), stmt.span)
            }
            ...
            is CstForStmt -> {
                YxFor(stmt.varName.text, convertExpr(stmt.iterable), convertStmt(stmt.body), stmt.span)
            }

            is CstWhileStmt -> {
                YxWhile(convertExpr(stmt.condition), convertStmt(stmt.body), stmt.span)
            }

            is CstReturnStmt -> {
                YxReturn(stmt.value?.let { convertExpr(it) }, stmt.span)
            }
            ...
            is CstAsyncStmt -> {
                YxAsyncBlock(convertBlock(stmt.block), stmt.span)
            }

            is CstParallelStmt -> {
                YxParallelBlock(convertBlock(stmt.block), stmt.span)
            }

            is CstUnsafeStmt -> {
                YxUnsafeBlock(convertBlock(stmt.block), stmt.span)
            }
        }

    /** 公开转换入口：扩展关键字解析器下沉用（M8，把 `parseBlock()` 产出的块转为 AST）。 */
    fun convertBlock(block: CstBlock): YxBlock = YxBlock(block.statements.map { convertStmt(it) }, block.span)

    private fun convertWhenCondition(condition: CstWhenCondition): YxWhenConditionX =
        when (condition) {
    ...
    private fun convertExpr(expr: CstExpr): YxExpr =
        when (expr) {
            is CstIntLiteral -> {
                YxIntLiteral(expr.token.text, expr.span)
            }

            is CstFloatLiteral -> {
                YxFloatLiteral(expr.token.text, expr.span)
            }

            is CstCharLiteral -> {
                YxCharLiteral(expr.token.text, expr.span)
            }

            is CstBoolLiteral -> {
                YxBoolLiteral(expr.token.text == "true", expr.span)
            }

            is CstNullLiteral -> {
                YxNullLiteral(expr.span)
            }

            is CstRawStringLiteral -> {
                YxRawStringLiteral(expr.token.text, expr.span)
            }

            is CstStringLiteral -> {
                YxStringLiteral(expr.token.text, expr.span)
            }

            is CstStringText -> {
                YxStringText(expr.token.text, expr.span)
            }

            is CstStringTemplate -> {
                YxStringTemplate(expr.parts.map { convertExpr(it) }, expr.span)
            }

            is CstIdentifier -> {
                YxIdentifier(expr.token.text, expr.span)
            }
            ...
            is CstTypeReference -> {
                YxTypeReference(convertType(expr.type), expr.span)
            }

            is CstMemberAccess -> {
                YxMemberAccess(convertExpr(expr.receiver), expr.name.text, expr.span)
            }
            ...
            is CstTypeCall -> {
                YxTypeCall(convertType(expr.type), expr.args.map { convertExpr(it) }, expr.span)
            }

            is CstBinary -> {
                YxBinary(expr.op.text, convertExpr(expr.left), convertExpr(expr.right), expr.span)
            }

            is CstUnary -> {
                YxUnary(expr.op.text, convertExpr(expr.operand), expr.span)
            }

            is CstAwait -> {
                YxAwait(convertExpr(expr.operand), expr.span)
            }

            is CstLambda -> {
                YxLambda(expr.params.map { it.text }, convertExpr(expr.body), expr.span)
            }

            is CstBlockLambda -> {
                YxBlockLambda(convertBlock(expr.block), expr.span)
            }

            is CstParen -> {
                YxParen(convertExpr(expr.expr), expr.span)
            }

            is CstThrow -> {
                YxThrow(convertExpr(expr.expr), expr.span)
            }

            is CstIsExpr -> {
                YxIs(convertExpr(expr.expr), convertType(expr.type), expr.span)
            }

            is CstAsExpr -> {
                YxAs(convertExpr(expr.expr), convertType(expr.type), expr.span)
            }

            is CstNullable -> {
                YxNullable(convertExpr(expr.expr), expr.span)
            }

            is CstRangeExpr -> {
                YxRange(convertExpr(expr.from), convertExpr(expr.to), expr.span)
            }

            is CstAssignmentExpr -> {
                YxAssign(convertExpr(expr.target), expr.op.text, convertExpr(expr.value), expr.span)
            }
            ...
            is CstIndexExpr -> {
                YxIndexExpr(convertExpr(expr.base), convertExpr(expr.index), expr.span)
            }
        }
}
