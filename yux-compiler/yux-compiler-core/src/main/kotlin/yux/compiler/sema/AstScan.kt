...
object AstScan {
    ...
    fun visit(
        ...
        when (node) {
            is YxExpr -> {
                visitExpr(node, onNode)
            }

            is YxStmt -> {
                visitStmt(node, onNode)
            }
    ...
    private fun visitExpr(
        ...
        when (e) {
            ...
            is yux.compiler.ast.YxBlockLambda -> {
                visit(e.body, onNode)
            }

            is yux.compiler.ast.YxLambda -> {
                visit(e.body, onNode)
            }

            is yux.compiler.ast.YxAwait -> {
                visit(e.operand, onNode)
            }

            is yux.compiler.ast.YxStringTemplate -> {
                e.parts.forEach { visit(it, onNode) }
            }

            is yux.compiler.ast.YxMemberAccess -> {
                visit(e.receiver, onNode)
            }
            ...
            is yux.compiler.ast.YxTypeCall -> {
                e.args.forEach { visit(it, onNode) }
            }
            ...
            is yux.compiler.ast.YxUnary -> {
                visit(e.operand, onNode)
            }

            is yux.compiler.ast.YxParen -> {
                visit(e.expr, onNode)
            }

            is yux.compiler.ast.YxIs -> {
                visit(e.expr, onNode)
            }

            is yux.compiler.ast.YxAs -> {
                visit(e.expr, onNode)
            }

            is yux.compiler.ast.YxNullable -> {
                visit(e.expr, onNode)
            }
            ...
            is yux.compiler.ast.YxTypeReference -> {
                Unit
            }
    ...
    private fun visitStmt(
        ...
        when (s) {
            ...
            is yux.compiler.ast.YxBlock -> {
                s.statements.forEach { visit(it, onNode) }
            }

            is yux.compiler.ast.YxVarDecl -> {
                s.initializer?.let { visit(it, onNode) }
            }

            is yux.compiler.ast.YxExprStmt -> {
                visit(s.expr, onNode)
            }
            ...
            is yux.compiler.ast.YxReturn -> {
                s.value?.let { visit(it, onNode) }
            }

            is yux.compiler.ast.YxBreak,
            is yux.compiler.ast.YxContinue,
            -> {
                Unit
            }
            ...
            is yux.compiler.ast.YxAsyncBlock -> {
                visit(s.body, onNode)
            }

            is yux.compiler.ast.YxParallelBlock -> {
                visit(s.body, onNode)
            }

            is yux.compiler.ast.YxUnsafeBlock -> {
                visit(s.body, onNode)
            }
        }
    }
